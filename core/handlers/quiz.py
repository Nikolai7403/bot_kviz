...
async def get_resualt(user_id, bot: Bot, point):
    if point >= 8:
        ...
        text = f"Класс! {point} баллов — хороший результат.  А за прохождение квиза дарим сумку с гусями или брендированную бутылку. Забрать подарок можешь на стенде Звука."
        await bot.send_message(user_id, text=f"{text}\n{discription['more']}")
